class LineCoverage
  def self.local_coverage
    Hash[SimpleCov.result.files.map do |file|
      [
  ...
  def self.compare(argv)
    ...
    down_filtereds.each do |file, info|
      per_lines_less_than_100 = info[:covered_lines] < 100
      per_branches_less_than_100 = info[:covered_branches] < 100
      # puts "Cobertura Branches: #{per_branches_less_than_100}"
      per_lines_ok = !per_lines_less_than_100

      next if runing_all_test && per_lines_ok

      color = per_lines_less_than_100 ? :red : :cyan
      coverage_line = per_lines_less_than_100 ? info[:covered_lines] : 0.0
      coverage_branch = per_branches_less_than_100 ? info[:covered_branches] : 0.0
